...
let OrdersController = class OrdersController {
    ...
    async getMySales(req, limit, offset) {
        const user = req.user;
        return this.ordersService.getSellerSales(user.id, limit ? parseInt(limit) : 20, offset ? parseInt(offset) : 0);
    }
    async getMyPurchases(req, limit, offset) {
        const user = req.user;
        return this.ordersService.getBuyerPurchases(user.id, limit ? parseInt(limit) : 20, offset ? parseInt(offset) : 0);
    }
    async getMyStats(req) {
        const user = req.user;
        return this.ordersService.getSellerStats(user.id);
    }
    async getRecentSales(req, limit) {
        const user = req.user;
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, String, String]),
    __metadata("design:returntype", Promise)
], OrdersController.prototype, "getMyPurchases", null);
__decorate([
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], OrdersController.prototype, "getRecentSales", null);
__decorate([
    (0, common_1.Get)(':id'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
